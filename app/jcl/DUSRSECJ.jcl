...
//*-------------------------------------------------------------------*
//* SECURITY NOTE:
//*   The user-security records below contain SHA-256 hashed passwords
//*   plus per-user salts; plaintext passwords are NEVER stored in the
//*   security file.  The hashes are demo-only digests of the well-
//*   known password "PASSWORD" intended for development sandboxes.
//*
//*   For production deployments use the bootstrap script
//*       db/migrate/init_user_security.sh
//*   which sources the bootstrap admin / user passwords from AWS
//*   Secrets Manager (or any compatible secret store) and writes the
//*   hashed records to either the modern user_security relational
//*   table or this VSAM dataset.  See config/secrets-config.md.
//*
//*   Record layout: copybook CSUSR01Y (record length = 160)
//*       USR-ID(8) FNAME(20) LNAME(20) LEGACY-PWD(8 - blank)
